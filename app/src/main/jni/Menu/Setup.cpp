...
void setDialog(jobject ctx, JNIEnv *env, const char *title, const char *msg)
{
    ...
    jmethodID setTitle = env->GetMethodID(Alert, OBFUSCATE("setTitle"),
    ...
    jmethodID setMsg = env->GetMethodID(Alert, OBFUSCATE("setMessage"),
                                        OBFUSCATE("(Ljava/lang/CharSequence;)Landroid/app/AlertDialog$Builder;"));
    env->CallObjectMethod(MainAlert, setMsg, env->NewStringUTF(msg));

    jmethodID setCa =
        env->GetMethodID(Alert, OBFUSCATE("setCancelable"), OBFUSCATE("(Z)Landroid/app/AlertDialog$Builder;"));
    env->CallObjectMethod(MainAlert, setCa, false);

    jmethodID setPB = env->GetMethodID(Alert, OBFUSCATE("setPositiveButton"),
                                       OBFUSCATE("(Ljava/lang/CharSequence;Landroid/content/"
                                                 "DialogInterface$OnClickListener;)Landroid/app/AlertDialog$Builder;"));
...
void startActivityPermisson(JNIEnv *env, jobject ctx)
{
    ...
    jmethodID startActivity =
        env->GetMethodID(native_context, OBFUSCATE("startActivity"), OBFUSCATE("(Landroid/content/Intent;)V"));
    ...
    jmethodID Parce =
        env->GetStaticMethodID(Uri, OBFUSCATE("parse"), OBFUSCATE("(Ljava/lang/String;)Landroid/net/Uri;"));
    ...
    jmethodID newIntent =
        env->GetMethodID(intentclass, OBFUSCATE("<init>"), OBFUSCATE("(Ljava/lang/String;Landroid/net/Uri;)V"));
    jobject intent =
        env->NewObject(intentclass, newIntent,
                       env->NewStringUTF(OBFUSCATE("android.settings.action.MANAGE_OVERLAY_PERMISSION")), UriMethod);
...
void startService(JNIEnv *env, jobject ctx)
{
    ...
    jmethodID newIntent =
        env->GetMethodID(intentClass, OBFUSCATE("<init>"), OBFUSCATE("(Landroid/content/Context;Ljava/lang/Class;)V"));
    jobject intent = env->NewObject(intentClass, newIntent, ctx, actionString);
    jmethodID startActivityMethodId =
        env->GetMethodID(native_context, OBFUSCATE("startService"),
...
void CheckOverlayPermission(JNIEnv *env, jclass thiz, jobject ctx)
{
    ...
    { // Android 6.0
        ...
        jmethodID canDraw =
            env->GetStaticMethodID(Settings, OBFUSCATE("canDrawOverlays"), OBFUSCATE("(Landroid/content/Context;)Z"));
    ...
    LOGI(OBFUSCATE("Decided service"));
...
void Init(JNIEnv *env, jobject thiz, jobject ctx, jobject title, jobject subtitle)
{
    ...
    setText(env, subtitle,
            OBFUSCATE("<b><marquee><p style=\"font-size:30\">"
                      "Modded by HitMargin | Thanks TheExploration | Punctulate TrueMLGPro</p>"
                      "</marquee></b>"));
    ...
    // Toast Example
    Toast(env, ctx, OBFUSCATE("Modded"), ToastLength::LENGTH_LONG);
